...
function App() {
  ...
  const [quickTestForm, setQuickTestForm] = useState({
    ...
    service: 'tts',
    models: {
      elevenlabs: { tts_model: 'eleven_flash_v2_5', stt_model: 'scribe_v1', voice_id: 'Xb7hH8MSUJpSbSDYk0k2' },
  ...
  const RunResultCard = ({ run, filters = { vendor: 'all', service: 'all', model: 'all' } }) => {
    ...
    const RatingModal = ({ item, isOpen, onClose, onRatingUpdate }) => {
      ...
      // For e2e items, prefer STT metrics (so STT-specific metrics like Noise Handling appear)
      const actualServiceType = serviceType === 'e2e' ? 'stt' : serviceType;
